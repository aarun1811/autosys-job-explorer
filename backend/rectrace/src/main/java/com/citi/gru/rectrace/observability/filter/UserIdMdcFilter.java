...
/**
 * OBS-01 / D-7.11 — populates the {@code userId} MDC key from the
 * {@code x-citiportal-loginid} request header so the structured access log
 * line emitted by {@link AccessLogFilter} can be attributed back to a user.
 *
 ...
 * regex {@code ^[A-Za-z0-9._-]{1,64}$} is the trust-boundary check:
 * <ul>
 *   <li>Alphanumerics + dot + underscore + dash cover real Citi loginIds
 *       (e.g. {@code arun.tech_01}).</li>
 *   <li>Length cap 64 chars prevents oversized headers being copied into
 *       every subsequent log line.</li>
 *   <li>Rejected values are silently dropped (no MDC.put, no log warning).
 *       Silent rejection preserves anonymous-probe behaviour and avoids
 *       generating a log line that itself echoes attacker-controlled text.</li>
 * </ul>
 *
 ...
 * request handled by the same thread. The remove is unconditional: even if
 * this request did not set a userId, removing in finally defends against
 * a leak from a prior request that ran on the same thread before this
 * filter (e.g. an in-flight async response).
 */
@Component
