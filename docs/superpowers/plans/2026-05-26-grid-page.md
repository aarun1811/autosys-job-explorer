...
Also drop `useRef` from the React import — its only user (`gridApiRef`) is removed in (b), so leaving it trips `@typescript-eslint/no-unused-vars`. Change the first line:
...
- **Do NOT "simplify" the URL-sync effect** in `SearchPage` (the existing `useEffect` near lines 93-102 that does `search: (prev) => ({ ...prev, tab: activeCategory.key })`). Spreading `prev` is deliberate: it preserves a deep-linked `view` through the first auto-tab-selection so a shared link restores. `handleSelectTab` is what intentionally drops `view` on an explicit tab switch.
- **`isExporting` spinner is decorative.** Excel export is synchronous, so React batches `setIsExporting(true)`→`false` in one tick and the `Loader2Icon` never animates (matches the old `SearchPage.handleExport`). Don't chase it as a bug.
- **Keep `RowApiModule` registered** — the status-bar row-count panels use the row API. Its main.tsx comment still references the now-removed "result-count Badge"; update that comment when editing main.tsx (Task 6), but do not remove the module.
- **`window.location.href` in `onShare`** is correct in the real browser (TanStack Router uses browser history). In the memory-router unit test it resolves to the jsdom default, so the test only asserts the clipboard was called once — not the copied value.
